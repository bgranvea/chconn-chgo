...
func intColumns(n int) []*column.Nullable[int64] {
	res := make([]*column.Nullable[int64], n)
	for i := 0; i < n; i++ {
...
func strColumns(n int) []*column.Nullable[string] {
	res := make([]*column.Nullable[string], n)
	for i := 0; i < n; i++ {
